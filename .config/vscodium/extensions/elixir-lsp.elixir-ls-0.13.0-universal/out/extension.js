...
function startClient(context, clientOptions) {
    ...
    const lsOverridePath = vscode.workspace.getConfiguration('elixirLS').get('languageServerOverridePath');
    const serverOpts = {
...
function configureTestController(context) {
    ...
    (function (ItemType) {
        ...
        ItemType[ItemType["Module"] = 1] = "Module";
        ItemType[ItemType["Describe"] = 2] = "Describe";
        ItemType[ItemType["TestCase"] = 3] = "TestCase";
    })(ItemType || (ItemType = {}));
        ...
        while (queue.length > 0 && !token.isCancellationRequested) {
            ...
            switch (getType(test)) {
                ...
                case ItemType.TestCase:
                    ...
                    run.started(test);
                    try {
                        let folder = vscode_1.workspace.getWorkspaceFolder(test.uri);
                        folder = getOuterMostWorkspaceFolder(folder);
                        const folderPath = folder.uri.fsPath;
                        const relativePath = test.uri.fsPath.slice(folderPath.length + 1);
                        const output = await (0, runTest_1.default)({ cwd: folderPath, filePath: relativePath, line: test.range.start.line + 1 });
